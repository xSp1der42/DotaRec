...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // либо сработает таймер через 15 секунд.
    Promise.race([
      getSessionAndProfile(),
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Проверка авторизации заняла слишком много времени')), 15000) // ИЗМЕНЕНО: 5000 -> 15000
      )
    ]).catch(error => {
      // Логируем ошибку, если она была (включая таймаут)
      console.warn('Проблема при инициализации AuthProvider:', error.message);
    }).finally(() => {
      // Этот блок выполнится в любом случае - при успехе, ошибке или таймауте.

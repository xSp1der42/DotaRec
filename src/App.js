  ...
  const fetchData = useCallback(() => {
    ...
    Promise.race([
      ...
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Загрузка публичных данных заняла слишком много времени')), 15000) // ИЗМЕНЕНО: 8000 -> 15000
      )
    ]).catch(error => {
      console.warn('Проблема при загрузке данных приложения:', error.message);
    }).finally(() => {
      // Этот блок гарантирует, что загрузка прекратится в любом случае.
      setAppLoading(false);
    });
  }, []);

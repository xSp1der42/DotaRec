...
function App() {
  ...
  // --- USEEFFECT ДЛЯ СОХРАНЕНИЯ ДАННЫХ ---
  ...
  const handleAddCoins = (amount) => {
    ...
    setUserCoins(prev => prev + numericAmount);
    alert(`${numericAmount.toLocaleString('ru-RU')} коинов успешно добавлено на ваш баланс!`);
  };
  ...
  const uploadPlayerImage = async (file) => {
    ...
    const { data } = supabase.storage.from('player_avatars').getPublicUrl(fileName);
  ...
  const handleUpdatePlayer = async (updatedPlayerData, imageFile) => {
    ...
    if (imageFile) imageUrl = await uploadPlayerImage(imageFile);
  ...
  const handleOpenPack = (packId) => {
    ...
    if (!pack || userCoins < pack.price) return null;
  ...
  if (loadingApp) return <div className="App"><header className="app-header"><h2>Загрузка...</h2></header></div>;
        ...
        <div className="header-right">
          {!isAnyAdminView && (
            <div className="user-coins">
              {userCoins.toLocaleString('ru-RU')} коинов
            </div>
          )}
          {session ? <button className="admin-toggle-button" onClick={handleLogout}>Выйти</button> : <NavLink to="/login" className="admin-toggle-button">Войти</NavLink>}
        </div>
      </header>

...
// Иконка для перетаскивания (просто для наглядности)
...
export const SortablePlayerItem = ({ player, onEdit, onDelete }) => {
  const {
    ...
    transform,
    transition,
    isDragging, // Состояние, когда элемент перетаскивается
  ...
  return (
    <div ref={setNodeRef} style={style} className="player-item-admin">
      {/* Ручка для перетаскивания. Вешаем на нее слушатели событий */}
